...
def _save_and_upload_images(images, job_id):
    ...
    for index, image in enumerate(images):
        image_path = os.path.join(f"/{job_id}", f"{index}.jpg")
        # Save as JPEG with quality cap — keeps base64 payload under RunPod's
        # job-done size limit when BUCKET_ENDPOINT_URL isn't configured.
